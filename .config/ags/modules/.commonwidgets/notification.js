...
const getFriendlyNotifTimeString = (timeObject) => {
  ...
  else return messageTime.format(userOptions.asyncGet().time.dateFormat);
};
...
export default ({ notifObject, isPopup = false, props = {} } = {}) => {
      ...
      initTimeString == "Now"
        ? (self) => {
            ...
            if (initTimeString == "Now") {
              self.attribute.timeoutId = GLib.timeout_add(
                ...
                () => {
                  ...
                  try {
                    ...
                    console.log(
                      `Error updating notification time: ${e.message}`
                    );
                  }
        ...
        .hook(
          ...
          (self) => {
            ...
            if (
              ...
              Utils.timeout(
                userOptions.asyncGet().animations.durationSmall * 2,
                () => {
                  command();
                  if (wholeThing) {
                    wholeThing.destroy();
  ...
  gesture.connect("cancel", () => {
    try {
      ...
      if (wholeThing?.attribute) wholeThing.attribute.dragging = false;
