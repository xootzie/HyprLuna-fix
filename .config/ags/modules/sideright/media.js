...
// Reference to the media container for updating background
let mediaContainer = null;

// Function to set the media container reference
const setMediaContainer = (container) => {
...
const MediaControls = () => {
    ...
    const updateControls = () => {
        ...
        if (playerMode === 'offline' && lastPlayedFile && !player.getCurrentFile()) {
            ...
            if (mediaContainer) {
                mediaContainer.css = 'background-image: none;';
            }
        ...
        if (player.getCurrentFile() && playerMode === 'offline') {
            ...
            // Clear background image for offline mode
            if (mediaContainer) {
                mediaContainer.css = 'background-image: none;';
            }
        ...
        if (mprisPlayer) {
            ...
            // Set background image from track cover if available
            if (mediaContainer && mprisPlayer.trackCoverUrl) {
                try {
                    const coverUrl = mprisPlayer.trackCoverUrl;
                    if (coverUrl && coverUrl !== '') {
                        mediaContainer.css = `background-image: url('${coverUrl}');`;
                    } else {
                        mediaContainer.css = 'background-image: none;';
                    }
                } catch (e) {
                    console.error('Error setting cover art as background:', e);
                    mediaContainer.css = 'background-image: none;';
                }
    ...
    return Widget.Box({
        ...
        children: [
            ...
            Widget.Box({
                vexpand: true, // Take up available space
            }),
            // Bottom container for progress bar and buttons
            Widget.Box({
                vertical: true,
                className: 'media-bottom-container',
                children: [
                    ...
                    Widget.Box({
                        spacing: 8, // Reduced spacing for more buttons in a row
        ...
        children: [
            ...
            Widget.Box({
                ...
                setup: self => {
                    ...
                    // Pass reference to the media container for background image updates
                    setMediaContainer(self);

                    // Update controls to set initial background if needed
                    if (typeof updateControlsCallback === 'function') {
                        updateControlsCallback();
                    }
                },
